...
type Skill = {
  name: string;
  color: string;
  /** Optional 24x24 SVG logo path (Simple Icons). Falls back to text. */
  path?: string;
};

// A broad skill set — most sit at the back (faded/hidden) and rotate into view.
const skills: Skill[] = [
  {
    name: "React",
    color: "#149eca",
  ...
  { name: "JavaScript", color: "#a98a00" },
  { name: "Go", color: "#0094b3" },
  { name: "Docker", color: "#1d72c4" },
  { name: "Kubernetes", color: "#326ce5" },
  { name: "AWS", color: "#d97706" },
  { name: "Vue", color: "#369870" },
  { name: "Java", color: "#c74634" },
  { name: "GraphQL", color: "#c4007e" },
  { name: "Rust", color: "#9a5430" },
  { name: "Flutter", color: "#0b81d6" },
];

const TAU = Math.PI * 2;
const IDLE_SPEED = 0.12; // radians / second when idle
const DRAG_SENSITIVITY = 0.009;

export function TechOrbit() {
  const [, force] = useState(0);
  const angleRef = useRef(Math.PI); // start with the first skill at front
    ...
    const tick = (now: number) => {
      ...
      if (!draggingRef.current) {
        if (reduceRef.current) {
          ...
          // Idle drift plus decaying momentum from the last drag.
          angleRef.current += IDLE_SPEED * dt + velocityRef.current * dt;
          velocityRef.current *= 0.94;
          if (Math.abs(velocityRef.current) < 0.001) velocityRef.current = 0;
        }
      }
      if (angleRef.current > TAU) angleRef.current -= TAU;
      if (angleRef.current < 0) angleRef.current += TAU;
  ...
  return (
    ...
    >
      {skills.map((skill, i) => {
        ...
        const x = Math.sin(a); // -1 (left/back) .. 1 (right/front of arc)
        const c = Math.cos(a); // 1 top .. -1 bottom

        // Only the UPPER-RIGHT arc is visible. Fade horizontally (back-left)
        // and vertically (anything dropping toward the desk) so no ghost chips
        // leak over the laptop/figure.
        const visX = clamp01((x + 0.1) / 0.6);
        const visY = clamp01((c + 0.16) / 0.4);
        const vis = Math.min(visX, visY);
        if (vis <= 0.05) return null;

        const scale = 0.78 + vis * 0.42;
        const blur = vis < 0.5 ? (0.5 - vis) * 4 : 0;
        const z = Math.round(vis * 6) + 1; // banner sits at z-index 2

        // Bias the arc to the right so it hugs the figure's edge.
        const left = 62 + x * 38;
        const top = 42 - c * 34;
