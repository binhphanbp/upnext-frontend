...
import { adminDeletePostTag, adminGetPostTags, type FullPostTag } from "@/features/admin/api/posts";
...
import { PostTagFormDialog } from "./post-tag-form-dialog";

export function PostTagsTab() {
  ...
  const [editingTag, setEditingTag] = React.useState<FullPostTag | null>(null);

  const {
    data: tags = [],
    isLoading,
    refetch,
  } = useQuery<FullPostTag[]>({
    queryKey: ["adminPostTags"],
    queryFn: async () => {
      const session = getAdminSession();
      if (!session) throw new Error("No session");
      return adminGetPostTags(session.accessToken);
    },
  });
  ...
  const deleteMutation = useMutation({
    mutationFn: async (tag: FullPostTag) => {
      const session = getAdminSession();
      if (!session) throw new Error("No session");
      return adminDeletePostTag(session.accessToken, tag.id);
    },
    onSuccess: () => {
      void Swal.fire({
        icon: "success",
        title: "Đã xóa",
        text: "Đã xóa thẻ bài viết",
        timer: 1500,
        showConfirmButton: false,
      });
      void queryClient.invalidateQueries({ queryKey: ["adminPostTags"] });
    },
    onError: (err: any) => {
      void Swal.fire({
        icon: "error",
        title: "Không thể xóa",
        text: err?.message || "Có lỗi xảy ra khi xóa thẻ bài viết",
      });
    },
  });

  const handleDelete = async (tag: FullPostTag) => {
    ...
    const confirm = await Swal.fire({
      ...
      html: `
        <div class="text-sm text-slate-600 text-left space-y-1">
          <p>• Số bài viết đang gắn thẻ: <strong>${postCount} bài viết</strong></p>
          <p class="text-rose-600 mt-2">Hành động này sẽ gỡ thẻ khỏi tất cả bài viết liên quan.</p>
        </div>
      `,
      ...
      <div className="flex flex-col gap-3 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="relative w-full sm:w-[280px] md:w-[320px]">
          ...
          <Input
            ...
            placeholder="Tìm theo tên thẻ, slug..."
            value={searchTerm}
      ...
      <Card className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
            ...
            <table className="w-full min-w-[700px] border-collapse text-sm whitespace-nowrap">
              ...
              <tbody className="divide-y divide-slate-100 bg-white text-sm text-slate-700">
                {paginatedData.length === 0 ? (
                  ...
                  paginatedData.map((t) => {
                    const postCount = t._count?.postTags ?? 0;

                    return (
                      <tr key={t.id} className="transition-colors hover:bg-slate-50/80">
                        <td className="px-5 py-3.5">
                          <div className="flex items-center gap-2">
                            <span className="inline-flex items-center gap-1.5 rounded-lg bg-emerald-50 px-2.5 py-1 text-xs font-semibold text-emerald-700 ring-1 ring-emerald-600/20 ring-inset">
        ...
        <div className="flex flex-col gap-3 border-t border-slate-200 bg-slate-50/50 px-5 py-3.5 text-xs text-slate-600 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex items-center gap-2">
            ...
            <span>
              trên tổng số <strong>{totalItems.toLocaleString()}</strong> thẻ bài viết
            </span>
          </div>
